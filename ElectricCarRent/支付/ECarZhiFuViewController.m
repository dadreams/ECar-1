...
//    self.fkLabel1.center = jiaLabel3.center;
    self.fkLabel1.text = [NSString stringWithFormat:@"30元"];
    ...
    if (![WXApi isWXAppInstalled] || self.sendType == sendToHouTaiByVip) {
        wxLabel.hidden = YES;
...
//    order.amount = @"0.01";
    order.notifyURL =  @"http://www.dreamers-makers.com"; //回调URL
...
//                NSString * strTitle = [NSString stringWithFormat:@"支付结果"];
                ...
                [self showHUD:@"请稍等..."];
                [[self.mapManager zhifubaofinishOrderWithID:config.orignOrderNo andCanshu:resultDic[@"result"]] subscribeError:^(NSError *error) {
