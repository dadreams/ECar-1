...
@interface ECarInputViewController ()<UITextFieldDelegate, UISearchBarDelegate, AMapSearchDelegate>
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField{
    ...
    self.text.text.biaoShiTag = @(10000);
    [[AMapSearchManager instance] searchPlaceWithKeywords:self.text.text city:@"beijing" success:^(NSArray *ary) {
        [weakSelf hideHUD];
        if (weakSelf.chaoshi) {
            return ;
        }
        ...
        [weakSelf hideHUD];
        [weakSelf delayHidHUD:@"网络异常，请重新操作..."];
    }];
    return YES;
}
