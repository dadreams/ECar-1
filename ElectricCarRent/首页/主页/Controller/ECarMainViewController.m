...
@interface ECarMainViewController () <MAMapViewDelegate, AMapNaviManagerDelegate, IFlySpeechSynthesizerDelegate, ECarOrderViewControllerDelegate, ECarMyCenterViewControllerDelegate, AMapNaviViewControllerDelegate, UIAlertViewDelegate, UIScrollViewDelegate, ECarDrivingViewControllerDelegate>
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if ([ECarConfigs shareInstance].timeStr.length != 0) {
...
- (void)createOrder:(id)model success:(void(^)())block andFail:(void(^)(NSString * msg))failBlock
{
    ...
    ECarUser *userInfo = [ECarConfigs shareInstance].user;
    [ECarConfigs shareInstance].currentCarID = ecarInfo.carid;
    [[weakSelf.manager createOrder:ecarInfo.carid userID:userInfo.user_id begin:self.currentRoadInfo destination:self.userDestitation.name area:@"beijing" power:@"pd" endLatitude:FloatToString(self.userDestitation.location.latitude) endLongitude:FloatToString(self.userDestitation.location.longitude) userLatitude:FloatToString(userCoordinate.latitude) userLongitude:FloatToString(userCoordinate.longitude) theCarLatitude:self.carInfo.carLatitude theCarLongitude:self.carInfo.carlongitude andCarLocation:self.carInfo.locationName] subscribeNext:^(id x) {
        ...
        [ECarConfigs shareInstance].orignOrderNo = order;
        block();
    } error:^(NSError *error) {
        [weakSelf hideHUD];
...
- (void)getZheXianFanWeiKongZhi
{
    ...
    [[self.manager getZiDingYiFanWeiKongZhiWithVersion:version] subscribeNext:^(id x) {
        ...
        if (timeStr.length != 0) {
            ...
            [ECarConfigs shareInstance].timeStr = timeStr;
        ...
        if (attributes) {
            ...
            switch (type.integerValue) {
                ...
                case 3:
                {
                    ...
                    [ECarConfigs shareInstance].gengxinAlert = self.popAlertView;
                    [self.popAlertView show];
                    break;
                }
                default:
...
{
    weak_Self(self);
    NSString * userid = [ECarConfigs shareInstance].user.user_id;
    if (!userid) {
        userid = @"dashabi";
    }
...
- (void)wodeXingChengClicked:(UIButton *)sender
{
    ...
    /**
    if (![ECarConfigs shareInstance].loginStatue) {
...
- (BOOL)checkUserInfoWithUser:(ECarUser *)user
{
    ...
    if ([user.islock isEqualToString:@"1"]) {
        UIAlertView *alertV = [[UIAlertView alloc] initWithTitle:@"锁定" message:@"用户状态被锁定，如有疑问，请联系客服：400-8939091。" delegate:nil cancelButtonTitle:@"确认" otherButtonTitles:nil];
        [alertV show];
        return success;
    }
...
- (void)mapView:(MAMapView *)mapView didUpdateUserLocation:(MAUserLocation *)userLocation updatingLocation:(BOOL)updatingLocation
{
    if(updatingLocation)
    {
        if ([ECarConfigs shareInstance].loginStatue == YES && self.registerJpushFirst == NO) {
            [self.manager sendUserLocationWithPhone:[ECarConfigs shareInstance].user.phone latitude:[NSString stringWithFormat:@"%lf", userLocation.coordinate.latitude] longtitude:[NSString stringWithFormat:@"%lf", userLocation.coordinate.longitude]];
        ...
        [ECarConfigs shareInstance].userCoordinate = userLocation.coordinate;
...
- (void)naviManager:(AMapNaviManager *)naviManager didDismissNaviViewController:(UIViewController *)naviViewController
{
    self.persentNav = NO;
    // 退出导航界面后恢复地图的状态
    if ([ECarConfigs shareInstance].exitTag == ExitNaviTagOpenLockSuccess) {
        //开锁成之后出现行车导航按钮
        [self drivingNavi];
    }
}
...
- (void)addzhifuViewJieshu
{
    ...
    if (config.currentPrice.doubleValue < 0.001) {
...
{
    [ECarConfigs shareInstance].userCoordinate = CLLocationCoordinate2DMake(naviLocation.coordinate.latitude, naviLocation.coordinate.longitude);
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (alertView.tag == PopoverAlertTypeThree) {
        if (buttonIndex == 0) {
            ...
            [ECarConfigs shareInstance].gengxinAlert = nil;
        }
